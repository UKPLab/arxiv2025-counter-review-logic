...
class ActivePassiveCF(PaperCounterfactualGenerator):
    """
    This class generates counterfactuals by converting active voice to passive voice and vice versa. It is explicitly
    asked to not make any content changes whatsover.

    It is a manually designed counterfactual generator.
    """

    def __init__(self, llm: ChatLLM, prompt_base_path: Path | str=None, conversion_ratio=0.4):
        super().__init__(name=f"active_passive_{conversion_ratio:.2f}")
    ...
    def use_conversion_prompt(self):
        self.llm.load_prompt(self.prompt_base_path / "manual/style_change/active_passive_conversion.txt")
    ...
    def single_run(self, paper: Paper, **config) -> PaperCounterfactual:
        ...
        for section_name in section_names:
            ...
            for p_i, p_text in enumerate(paragraphs):
                # set to active-to-passive prompt
                self.use_conversion_prompt()
                inputs += [{
                    "paper": p_text
                }]
        ...
        if self.conversion_ratio < 1.0:
            subset_inputs = random.sample(range(len(inputs)), int(len(inputs) * self.conversion_ratio))
        ...
        revised = self.llm([{
            "incorrect_response": revision_suggestions[i][0]  # pass the raw answer, not the failed parse (which is none)
        ...
        for i in range(len(needs_revision)):
            ...
            if revision_suggestion is not None:
                ...
                if valid_output:
                    ...
                    logging.debug(
                        f"Paper {paper.id} section {section_name} could not be revised from passive to active. Output error...")
        ...
        return PaperCounterfactual(
            ...
            cf_paper=cf_paper,
            changes={
                "highlevel": [{"target": "sec_" + section_name, "change": change} for section_name, change in changes.items()] \
